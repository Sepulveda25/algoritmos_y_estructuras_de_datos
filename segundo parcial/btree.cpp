...
class arbol:archivo
{
        ...
        public :
                ...
                int crearArbolBusqueda ( int val, nodo *n, int *p, nodo **c, FILE *arch) ;
                ...
                void explota ( int val, nodo *hijo, nodo *node, int k, int *nuevo_padre, nodo **nuevo_hijo, FILE *arch, int &mx) ;
                void borrar ( int val ) ;
...
void arbol :: insertar ( int val, FILE *arch)
{
        ...
        nodo *c, *n ;
        int flag ;
        flag = crearArbolBusqueda ( val, raiz, &i, &c, arch) ;
        if ( flag )
        {
                n = new nodo ;
                n -> tamanio = 1 ;
...
int arbol :: crearArbolBusqueda ( int val, nodo *node, int *padre, nodo **hijo, FILE *arch)
{
        ...
        {       cout<<"pas else";
                ...
                if ( crearArbolBusqueda ( val, node -> hijo[k], padre, hijo, arch ) )
                {
                        cout<<"HOLA!!!!"<<endl;
                        if ( node->tamanio < MAX )
                        {
                        ...
                        {
                                explota ( *padre, *hijo, node, k, padre, hijo, arch, mx) ;
                                return 1 ;
                        }
                }
...
void arbol :: explota ( int val, nodo *hijo, nodo *node, int k, int *nuevo_padre, nodo **nuevo_hijo, FILE *arch, int &mx)
{
        struct archivo dato;
        int i, mid ;
        int large_d,large_p;
